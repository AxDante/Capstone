void threadSerial(){
  ...
  if (Serial2.available()>0) {
   ...
   // Input = h   -- LED green
    ...
    //Input = p   -- Camera Resolution = small
    else if (incomingByte == 112){
        lcd.clear();
        lcd.setCursor(0, 0);
        char toPrint[16] = "Cam Res: Low";
        lcd.print(toPrint);
        cam.setImageSize(VC0706_160x120);          // low
        inputTime = millis();
    }

    //Input = q   -- Camera Resolution = medium
    else if (incomingByte == 113){
        lcd.clear();
        lcd.setCursor(0, 0);
        char toPrint[16] = "Cam Res: Med";
        lcd.print(toPrint);
        cam.setImageSize(VC0706_320x240);          // medium
        inputTime = millis();
    }

    //Input = r   -- Camera Resolution = medium
    else if (incomingByte == 114){
        lcd.clear();
        lcd.setCursor(0, 0);
        char toPrint[16] = "Cam Res: High";
        lcd.print(toPrint);
        cam.setImageSize(VC0706_640x480);          // high
        inputTime = millis();
    }
    
  }else if (millis() - inputTime > 10000){
      lcd.clear();
      lcd.setCursor(0, 0);
